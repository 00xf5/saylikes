...
class LikeAccessibilityService : AccessibilityService() {
    ...
    fun startBot(maxLikes: Int, loginWithin: String, speed: String) {
        ...
        try {
            overlay?.show(maxLikes)
            overlay?.setCount(0, maxLikes)
        } catch (_: Exception) {
        }
        worker?.post {
            var liked = 0
            ...
            try {
                ...
                try {
                    RunStore.saveFinished(applicationContext, liked, names)
                    if (liked > 0) {
                        val sub = RunStore.isSubscription(applicationContext)
                        val left = TrialStore.consume(applicationContext, liked, sub)
                        log("Trial remaining: $left")
                    }
                } catch (e: Exception) {
                    log("Save result error: ${e.message}")
                }
    ...
    private fun runBot(
        ...
        var idleScrolls = 0
        try {
            overlay?.setCount(liked, maxLikes)
        } catch (_: Exception) {
        }
